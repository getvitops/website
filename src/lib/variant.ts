 ...
 *
 * CURRENTLY DORMANT. The system is intact but no B content exists, and
 * DEFAULT_SPLIT_B is 0 so every visitor gets A. To run an experiment: add the
 * B content under `src/_b/`, then set the split. `?variant=b` still forces B
 * for review at any time, independent of the split.
 *
 ...
 *  0 while no B content exists — a live split would hand out `b` cookies and
 *  `x-ab-variant: b` attribution for an experiment that isn't running, and
 *  keep HTML on `private, no-store` for nothing. Raise it to launch a test. */
export const DEFAULT_SPLIT_B = 0;
...
export function resolveVariant(
  ...
  const override = url.searchParams.get("variant") ?? request.headers.get("x-ab-variant");
